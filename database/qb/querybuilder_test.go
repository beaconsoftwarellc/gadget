...
func TestQueryBuilderSelectQuery_WhereError(t *testing.T) {
	...
	_, _, err := query.SQL(NewLimitOffset[int]().SetLimit(NoLimit).SetOffset(0))
...
func TestQueryBuilderSelectQuery_WhereValue(t *testing.T) {
	...
	actual, values, err = query.SQL(NewLimitOffset[int]().SetLimit(12).SetOffset(5))
...
func TestQueryBuilderJoin(t *testing.T) {
	...
	actual, values, err := query.SQL(NewLimitOffset[int]().SetLimit(10).SetOffset(0))
	if assert.Equal(2, len(values)) {
...
func TestQueryBuilderOrderBy_SQL(t *testing.T) {
	...
	actual, values, err := Select(Person.ID, Person.Name).
		From(Person).
		OrderBy(Person.ID, Ascending).
		SQL(NewLimitOffset[int]().SetLimit(10).SetOffset(10))
	assert.NoError(err)
	assert.Empty(values)
...
func TestQueryBuilderOrderByMulti_SQL(t *testing.T) {
	...
	actual, values, err := query.SQL(NewLimitOffset[int]().SetLimit(10).SetOffset(10))
	assert.NoError(err)
	assert.Empty(values)
